...
- **정확한 채점 매핑**: `GradingService.gradeAnswersWithQuestionNumbers()` 메서드 추가
  ...
  - 학번을 직접 파일명으로 지정했어야 했던 부분을 ocr 인식으로 바로 저장되도록 변경
- **하위 호환성 유지**: 기존 `extractAnswersFromImage()` 메서드 유지

#### 9. **점수 분포 차트 데이터 조회 및 시각화 버그 수정**
- **점수 분포 차트가 렌더링되지 않던 문제 해결**:
  - 원인: 조회 조건 불일치로 인한 빈 데이터([]) 반환
